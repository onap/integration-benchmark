...
class CsarParser:
    ...
    def get_service_yaml_from_csar(self, csar_file):
        ...
        :return:
        """
        #tmpdir = './__tmp' + datetime.now().strftime('%Y%m%d%H%M%S')
#	tmpdir = './__tmp'
...
#            zip_ref.extractall(tmpdir)
        ...
        #extract the csar file of different nodes into different tmpdir accordingly to avoid conflict during concurrent instantiation
        #for brg csar file extraction
        if 'brg' in csar_file:
            brg_csar_tmpdir = './__brg_tmp'
            if not os.path.isdir(brg_csar_tmpdir):
                os.mkdir(brg_csar_tmpdir)
                with zipfile.ZipFile(csar_file, "r") as zip_ref:
                    zip_ref.extractall(brg_csar_tmpdir)
            yamldir = brg_csar_tmpdir + '/Definitions'

        #for vgw csar file extraction
        if 'rescust' in csar_file:
            vgw_csar_tmpdir = './__vgw_tmp'
            if not os.path.isdir(vgw_csar_tmpdir):
                os.mkdir(vgw_csar_tmpdir)
                with zipfile.ZipFile(csar_file, "r") as zip_ref:
                    zip_ref.extractall(vgw_csar_tmpdir)
            yamldir = vgw_csar_tmpdir + '/Definitions'

        #for other csar file extration
        if (not 'brg' in csar_file) and (not 'rescust' in csar_file):
            others_csar_tmpdir = './__others_tmp'
            if not os.path.isdir(others_csar_tmpdir):
                os.mkdir(others_csar_tmpdir)
                with zipfile.ZipFile(csar_file, "r") as zip_ref:
                    zip_ref.extractall(others_csar_tmpdir)
            yamldir = others_csar_tmpdir + '/Definitions'
