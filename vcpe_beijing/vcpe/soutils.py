...
class SoUtils:
	...
	self.logger.info(" =>> submit_create_req request body:")
        self.logger.info(json.dumps(req_json, indent=4, sort_keys=True))
        r = requests.post(url, headers=self.vcpecommon.so_headers, auth=self.vcpecommon.so_userpass, json=req_json)
        self.logger.debug(r)
        response = r.json()
	
	self.logger.info(' =>> response from requests.post() in submit_create_req() for ' + req_type + ' is: ')
	...
	self.logger.info('req_id of ' + req_type + ' is: ' + req_id)
        instance_id = response.get('requestReferences', {}).get('instanceId', '')
	self.logger.info('instance_id of ' + req_type + ' is: ' + instance_id)
